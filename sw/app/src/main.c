...
typedef enum { APP_STATE_INIT_HW = 0, APP_STATE_WAIT_INIT, APP_STATE_RUNNING } AppState_t;

static void Format_Temp_Hum(char *buf, int32_t t_x100, uint32_t h_x10) {
    int32_t temp = t_x100 / 10; 
    if (temp < 0) { buf[2] = '-'; temp = -temp; } else { buf[2] = ' '; }
    
    buf[0] = 'T'; buf[1] = ':';
    buf[3] = (char)('0' + ((temp / 100) % 10)); buf[4] = (char)('0' + ((temp / 10) % 10));
    buf[5] = '.'; buf[6] = (char)('0' + (temp % 10)); buf[7] = 'C'; buf[8] = ' '; 
    buf[9] = 'H'; buf[10] = ':';
    buf[11] = (char)('0' + ((h_x10 / 100) % 10)); buf[12] = (char)('0' + ((h_x10 / 10) % 10));
    buf[13] = '.'; buf[14] = (char)('0' + (h_x10 % 10)); buf[15] = '%'; buf[16] = '\0';
}

static void Format_Pressure(char *buf, uint32_t p_pa) {
    buf[0] = 'P'; buf[1] = ':'; buf[2] = ' ';
    buf[3] = (char)('0' + ((p_pa / 100000U) % 10U)); buf[4] = (char)('0' + ((p_pa / 10000U) % 10U));
    buf[5] = (char)('0' + ((p_pa / 1000U) % 10U)); buf[6] = (char)('0' + ((p_pa / 100U) % 10U));
    buf[7] = (char)('0' + ((p_pa / 10U) % 10U)); buf[8] = (char)('0' + (p_pa % 10U));
...
static void System_Update_Tick(void) {
    if ((TIMER_STATUS_REG & TIMER_STATUS_TO_BIT) != 0U) {
        ...
        if (time_ms >= 1000U) {
            time_ms = 0U; time_sec++;
            if (time_sec >= 60U) { time_sec = 0U; time_min++; if (time_min >= 60U) { time_min = 0U; time_hr++; } }
        }
    }
}
...
int main(void) {
    ...
    char str_row0[17]; char str_row1[17];
    ...
    for (;;) {
        ...
        switch (app_state) {
            case APP_STATE_INIT_HW:
                LCD_Init_Async(); AHT20_Init_Async(); BMP280_Init_Async();
                app_state = APP_STATE_WAIT_INIT;
                break;
            case APP_STATE_WAIT_INIT:
                if (LCD_Is_Ready()) { UART_Tx_String("[SYS] FSMs Inicializadas.\r\n"); app_state = APP_STATE_RUNNING; }
                break;
            case APP_STATE_RUNNING:
                if ((sys_uptime_ms - last_sensor_trigger) >= 2000U) {
                    (void)AHT20_Trigger_Measurement(); (void)BMP280_Trigger_Measurement();
                    show_pressure_screen = !show_pressure_screen; 
                ...
                if (LCD_Is_Ready() && ((sys_uptime_ms - last_lcd_update) >= 150U)) {
                    ...
                    if (bmp_data.valid && aht_data.valid) Format_Temp_Hum(str_row0, bmp_data.temp_c_x100, aht_data.hum_rh_x10);
                    else Format_Temp_Hum(str_row0, 0, 0); 
                    
                    if (show_pressure_screen) {
                        if (bmp_data.valid) Format_Pressure(str_row1, bmp_data.press_pa);
                    } else {
                        str_row1[0] = ' '; str_row1[1] = ' '; 
                ...
                break;
            default: app_state = APP_STATE_INIT_HW; break;
        }
    }
    return 0;
}